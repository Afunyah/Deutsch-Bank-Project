...
function WatchlistApprovalPage(props){
    ...
    async function updateSystemWithResponse(acceptOrReject) {
        setLoadingPost(true)
        setError(false)
        await fetch ('http://localhost:3001/api/workshops/approveWatchlist', {
            method: 'POST',
            credentials : 'include',
            headers: {
                'Content-Type': 'application/json',
        ...
        .then(async (response) => {
            if (response.status === 200) {
                ...
                if(acceptOrReject === "Accept"){
                    await fetch ('http://localhost:3001/api/workshops/getWatchlist', {
                        method: 'POST',
                        credentials : 'include',
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    .then((data)=>{
                        var lsFormat = (data.watchlist).map((elem)=>{
                            var obj = {Name : elem};
                            return obj;
                        }); 
                        localStorage.setItem('watchlist', JSON.stringify(lsFormat));
                    })
                }
                ...
                {!loadingGet && !loadingPost &&
                        <Stack spacing={4}>
                            <Box sx={{display: 'flex',justifyContent: 'center', pt: '5%'}}>
                                <Stack spacing={2}>
                                    <Typography variant="h4" textAlign={"center"}>Workshop Recommendation</Typography>
                                    <Typography variant="h5" textAlign={"center"}>Based on your watchlist, we recommend this workshop to you.</Typography>
                                </Stack>
                            </Box>
                            ...
                            <Grid container spacing={0}>
                                ...
                                <Grid item xs={4} sx={{ display: 'flex', justifyContent: 'center'}}>
                                    <Button text="Book Now" onClick={() => {updateSystemWithResponse("Accept")}}></Button>
                                </Grid>
                                <Grid item xs={4} sx={{ display: 'flex', justifyContent: 'center'}}>
            ...
            <>
                ...
                {!loadingGet && !loadingPost &&
                        <Stack spacing={4}>
                            <Box sx={{display: 'flex',justifyContent: 'center', pt: '5%'}}>
                                <Stack>
                                    <Typography variant="h5" textAlign={"center"}>Workshop Recommendation</Typography>    
                                    <Typography variant="h6" textAlign={"center"}>Based on your watchlist, we recommend this workshop to you.</Typography>
                                </Stack>
                            </Box>
                            <Typography>Mentor:</Typography>
                            <StyleChip label={mentor.first_name + " " + mentor.last_name}/>
...
export default WatchlistApprovalPage;